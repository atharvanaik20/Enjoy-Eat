...
class CustomerRegister extends Component {
    ...
    handleChange = (e) => {
        ...
        const regexp2 = /^[a-z0-9._%+-]+@[a-z.-]+\.[a-z]{2,}$/;

        const regexp3 = /^[1-9]{1}[0-9]{5,5}$/;

        switch (name) {
            case 'phoneNumber':
                if (!regexp.test(value)) {
                    errors.phoneNumbererror = "Invalid Mobile Number";
                    uphnflag = false;
                }
                else {
        ...
        this.setState({ errors, [name]: value, userphonevalid: uphnflag, passwordvalid: pwdflag, zipcodevalid: zipflag, emailIdvalid: mailflag }, () => { this.setState({ formvalid: uphnflag && pwdflag && zipflag && mailflag }) });
    }
    ...
    submitData = (e) => {
        ...
        fetch("http://localhost:8081/customer/register", reqOptions)
            .then(resp => resp.text())
            .then(data => {
                if (data.length !== 0) {
